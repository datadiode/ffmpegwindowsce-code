...
//ms999
static int DIB_SetVolume(_THIS, int nProc);
...
static SDL_AudioDevice *Audio_CreateDevice(int devindex)
{
	...
	this->SetVolume = DIB_SetVolume;//ms999 
...
//ms999
static int DIB_SetVolume(_THIS, int nVol){

	if(sound) {

		//apply percent from settings
		double dLeft = 0xFFFF * nVol /100.0;
		double dRight = 0xFFFF * nVol /100.0;

		if (waveOutSetVolume(sound, MAKELONG((int)dLeft, (int)dRight)) == MMSYSERR_NOERROR )
			return 0;
