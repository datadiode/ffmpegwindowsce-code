...
static int DX5_SetVolume(_THIS,int proc);//ms999 
...
static SDL_AudioDevice *Audio_CreateDevice(int devindex)
{
	...
	this->SetVolume = DX5_SetVolume;//ms999 
...
//ms999 
static int DX5_SetVolume(_THIS, int procent){
	int nBDMin = DSBVOLUME_MIN/2;
	long llvol = nBDMin - procent*(nBDMin)/100;
	int result=0;
	if (mixbuf){
		result=IDirectSoundBuffer_SetVolume(mixbuf, llvol);
		if(result!=DS_OK){
			SetDSerror("DirectSound GetCurrentPosition", result);
		}
	}
