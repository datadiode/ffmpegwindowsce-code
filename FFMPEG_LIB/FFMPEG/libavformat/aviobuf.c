...
offset_t url_fsize(ByteIOContext *s)
{
 ...
 //ms99   size = s->seek(s->opaque, 0, AVSEEK_SIZE);
	size = -1; 
    if(size<0){
        //ms99 if ((size = s->seek(s->opaque, -1, SEEK_END)) < 0)
        if ((size = s->seek(s->opaque, 0, SEEK_END)) < 0)
            return size;
        //ms99 size++;
...
static offset_t dyn_buf_seek(void *opaque, offset_t offset, int whence)
{
    ...
    if (whence == SEEK_CUR)
        ...
        offset += d->size;
 #ifdef UNDER_CE
   if (offset < 0 || offset > (int64_t)0x7fffffff)
#else
