...
#ifdef UNDER_CE
        if(st->time_base.num*(int64_t)1000 > st->time_base.den){
#else
...
#ifdef UNDER_CE
        }else if(st->codec->time_base.num*(int64_t)1000 > st->codec->time_base.den){
#else
...
int av_seek_frame_binary(AVFormatContext *s, int stream_index, int64_t target_ts, int flags){
    ...
    int64_t pos_min=0, pos_max=0, pos=0, pos_limit=0;
    int64_t ts_min=0, ts_max=0, ts=0;
    int index=0;
    AVStream *st=0;
...
static void av_estimate_timings(AVFormatContext *ic, offset_t old_offset)
{
	...
	if(ic->xing_time > 0 && use_estimated == 1)
		ic->duration = ic->xing_time;
...
#ifdef UNDER_CE
            if(   (st->codec->time_base.den >= (int64_t)101*st->codec->time_base.num || st->codec->codec_id == CODEC_ID_MPEG2VIDEO)
#else
...
//                    av_log(NULL, AV_LOG_ERROR, "%f\n", dur);
    ...
    for(i=0;i<ic->nb_streams;i++) {
        ...
        if (st->codec->codec_type == CODEC_TYPE_VIDEO) {
            ...
            if(duration_count[i]
#ifdef UNDER_CE
               && (st->codec->time_base.num*(int64_t)101 <= st->codec->time_base.den || st->codec->codec_id == CODEC_ID_MPEG2VIDEO) ){
#else
...
#ifdef UNDER_CE
    int64_t pts_mask = ((int64_t)2 << (st->pts_wrap_bits-1)) - 1;
#else
...
//Functions not supported under CE 
// FIXME :  
