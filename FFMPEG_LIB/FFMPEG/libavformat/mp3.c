...
static void id3v2_read_ttag(AVFormatContext *s, int taglen, char *dst, int dstlen)
{
    ...
    case 0:  /* ISO-8859-1 (0 - 255 maps directly into unicode) */
        q = dst;
        while(taglen--) {
            uint8_t tmp = (uint8_t)get_byte(&s->pb);
			*q++ = tmp;
...
static int mp3_read_probe(AVProbeData *p)
{
 ...
 static void mp3_parse_xing(AVFormatContext *s, AVStream *st)
 {
   ...
   int frames = -1; /* Total number of frames in file */
	...
	//avRe.num = spf;
	//avRe.den = c.sample_rate;
    //st->duration = av_rescale_q(frames, avRe,
    //                            st->time_base);

	{
		int m_nDuration = spf * frames;
#ifdef UNDER_CE
		s->xing_time = (double)m_nDuration/c.sample_rate*(int64_t)1000000;
#else
		s->xing_time = (double)m_nDuration/c.sample_rate*1000000LL;
#endif
		if(s->xing_time < 0)
			s->xing_time = 0;
	}
