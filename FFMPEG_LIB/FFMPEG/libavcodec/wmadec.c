...
static int decode_exp_vlc(WMACodecContext *s, int ch)
{
    ...
    if (s->version == 1) {
		...
		v = s->pow_00625_10[last_exp];//sorin
        max_scale = v;
        n = *ptr++;
        do {
            *q++ = v;
...
static int wma_decode_block(WMACodecContext *s)
{
    ...
    /* normalize */
    {
        ...
        if (s->version == 1) {
            //mdct_norm *= sqrt(n4);//sorin
			mdct_norm *= ff_sqrt(n4);
        }
    }
    ...
    for(ch = 0; ch < s->nb_channels; ch++) {
        if (s->channel_coded[ch]) {
            ...
            //mult = pow(10, total_gain * 0.05) / s->max_exponent[ch];//sorin
            ...
            if (s->use_noise_coding) {
                ...
                for(j=-1;j<n1;j++) {
                    ...
                    if (j >= 0 && s->high_band_coded[ch][j]) {
                        ...
                        ///mult1 = sqrt(exp_power[j] / exp_power[last_high_band]);//sorin
						mult1 = ff_sqrt(exp_power[j] / exp_power[last_high_band]);
                        /* XXX: use a table */
                        //mult1 = mult1 * pow(10, s->high_band_values[ch][j] * 0.05);//sorin
