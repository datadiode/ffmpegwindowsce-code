...
int ff_wma_init(AVCodecContext * avctx, int flags2)
{
    ...
    if (s->use_noise_coding) {
...
#else
        {
            ...
            float norm;
			float ms01;//ms99
			long double ms02;//ms99
			double ms03; //ms99
            seed = 1;
  //ms999          norm = (1.0 / (float)(1LL << 31)) * sqrt(3) * s->noise_mult;
			ms01 = sqrt((float)3);
			ms02 = sqrt(3); 
			ms03 = sqrt((double)4); 
#ifdef UNDER_CE
			norm = (1.0 / (float)((int64_t)1 << 31)) * (float)sqrt(3) * s->noise_mult;
#else
			norm = (1.0 / (float)(1LL << 31)) * (float)sqrt(3) * s->noise_mult;
#endif
            for(i=0;i<NOISE_TAB_SIZE;i++) {
                seed = seed * 314159 + 1;
